...
static int cdns_i3c_master_get_rr_slot(const struct device *dev, uint8_t dyn_addr)
{
	...
	for (uint8_t i = find_lsb_set(activedevs); i <= find_msb_set(activedevs); i++) {
		...
		if (activedevs & BIT(rr_idx)) {
			uint32_t rr = sys_read32(config->base + DEV_ID_RR0(rr_idx));
...
static int cdns_i3c_attach_device(const struct device *dev, struct i3c_device_desc *desc)
{
	 ...
	 * When assigned through ENTDAA, the dynamic address, bcr, dcr, and pid are all set
	 * in the RR along with setting the device as active.
	 */
...
static void i3c_cdns_deftgts_work_fn(struct k_work *work)
{
	...
	uint32_t slv_status1 = sys_read32(config->base + SLV_STATUS1);
	...
	/* count the number of ones in devs */
	deftgts->count = POPCOUNT(devs);
	/* The IP will 'skip' writing it self to the RR if it was in DEFTGTS */
	if (slv_status1 & SLV_STATUS1_HAS_DA) {
		deftgts->count--;
	}
	...
	for (uint8_t i = find_lsb_set(devs); i <= find_msb_set(devs); i++) {
		...
		if (devs & BIT(rr_idx)) {
			...
			/*
			 * if it received deftgts and never got a DA skip RR 0 which contains
			 * itself
			 */
			if (rr_idx == 0 &&
			    !((slv_status1 & SLV_STATUS1_HAS_DA) == SLV_STATUS1_HAS_DA)) {
				continue;
			}
	...
	LOG_HEXDUMP_INF((uint8_t *)deftgts,
