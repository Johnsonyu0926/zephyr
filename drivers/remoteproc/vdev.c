...
{
	LOG_INF("metal_io %s va=%x pa=%lx size=0x%x %x", name, region->virt, *region->physmap,  region->size, region->page_shift);
}
...
static int vdev_init(const struct device *dev)
{
	...
	tid = k_thread_create(&data->thread, data->stack,
						  ...
						  NULL, CONFIG_REMOTEPROC_THREAD_PRIORITY, 0, K_NO_WAIT);
