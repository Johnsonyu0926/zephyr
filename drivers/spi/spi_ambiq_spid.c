...
static int spi_ambiq_xfer(const struct device *dev, const struct spi_config *config)
{
	...
	while (1) {
		...
		if (spi_context_tx_on(ctx)) {
			...
			if (ctx->rx_buf) {
				if (ctx->rx_len < num_read) {
					LOG_ERR("SPID read buffer inadequate!");
					ret = -ENOMEM;
					goto end;
				}
				/* Read data from LRAM */
				memcpy(ctx->rx_buf, (uint8_t *)&am_hal_ios_pui8LRAM[1], num_read);
			} else {
				while (num_read) {
					uint32_t size = (num_read > AMBIQ_SPID_DUMMY_LENGTH)
								? AMBIQ_SPID_DUMMY_LENGTH
								: num_read;
					uint32_t offset = 0;
			...
			spi_context_update_rx(ctx, 1, num_read);
			spi_ambiq_inform(dev);
			goto end;
		} else {
			break;
...
static int spi_ambiq_init(const struct device *dev)
{
	...
	if (AM_HAL_STATUS_SUCCESS !=
	    am_hal_ios_initialize((cfg->base - REG_IOSLAVE_BASEADDR) / cfg->size,
				  &data->ios_handler)) {
