...
int bt_le_cs_read_local_supported_capabilities(struct bt_conn_le_cs_capabilities *ret)
{
	...
	ret->initiator_supported =
		rp->roles_supported & BT_HCI_LE_CS_INITIATOR_ROLE_MASK;
	ret->reflector_supported =
		rp->roles_supported & BT_HCI_LE_CS_REFLECTOR_ROLE_MASK;
	ret->mode_3_supported =
		rp->modes_supported & BT_HCI_LE_CS_MODES_SUPPORTED_MODE_3_MASK;
	...
	if (rp->rtt_random_payload_n) {
		if (rp->rtt_capability & BT_HCI_LE_CS_RTT_RANDOM_PAYLOAD_N_10NS_MASK) {
			ret->rtt_random_payload_precision =
				BT_CONN_LE_CS_RTT_RANDOM_PAYLOAD_10NS;
		} else {
			ret->rtt_random_payload_precision =
				BT_CONN_LE_CS_RTT_RANDOM_PAYLOAD_150NS;
	...
	ret->cs_sync_2m_phy_supported =
		rp->cs_sync_phys_supported & BT_HCI_LE_CS_SYNC_PHYS_2M_MASK;
