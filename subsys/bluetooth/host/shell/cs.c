...
static int cmd_read_local_supported_capabilities(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (err) {
		shell_error(
			sh,
			"bt_le_cs_read_local_supported_capabilities returned error %d",
			err);
...
static int cmd_write_cached_remote_supported_capabilities(const struct shell *sh,
...
static int cmd_set_channel_classification(const struct shell *sh, size_t argc, char *argv[])
{
	...
	for (int i = 0; i < 10; i++) {
		...
		if (err) {
			...
			shell_error(
				sh,
				"Could not parse input %d, Channel Classification[%d]",
				i,
				i);
...
static int cmd_set_procedure_parameters(const struct shell *sh, size_t argc, char *argv[])
{
	...
	params.snr_control_initiator = BT_LE_CS_TEST_INITIATOR_SNR_CONTROL_18dB;
	params.snr_control_initiator = BT_HCI_OP_LE_CS_TEST_REFLECTOR_SNR_18;
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(security_enable, NULL, "<None>",
			  cmd_security_enable, 1, 0),
	SHELL_CMD_ARG(set_channel_classification, NULL, "<Byte 0> <Byte 1> <Byte 2> <Byte 3> "
								  "<Byte 4> <Byte 5> <Byte 6> <Byte 7> <Byte 8> <Byte 9>",
			  cmd_set_channel_classification, 11, 0),
	SHELL_CMD_ARG(set_procedure_parameters, NULL, "<None>",
			  cmd_set_procedure_parameters, 1, 0),
