...
{
	struct bt_ccp_client_bearer *gtbs_bearer = NULL;
	uint8_t tbs_count = 0U;

	if (err != 0) {
		shell_error(ctx_shell, "Failed to discover TBS: %d", err);
		return;
	}
	...
	shell_info(ctx_shell, "Discovery completed for client %p with %s%u TBS bearers", client,
		   gtbs_bearer != NULL ? "GTBS and " : "", tbs_count);
}

#if defined(CONFIG_BT_TBS_CLIENT_BEARER_PROVIDER_NAME)
static void ccp_client_bearer_provider_name_cb(struct bt_ccp_client_bearer *bearer, int err,
					       const char *name)
{
	if (err != 0) {
		shell_error(ctx_shell, "Failed to read bearer %p name: %d", (void *)bearer, err);
		return;
	}

	shell_info(ctx_shell, "Bearer %p name: %s", (void *)bearer, name);
}
...
static int cmd_ccp_client_discover(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (!cb_registered) {
		...
		if (err != 0) {
			shell_error(sh, "Failed to register CCP client cbs (err %d)", err);
			return -ENOEXEC;
		}
	...
	err = bt_ccp_client_discover(default_conn, &clients[bt_conn_index(default_conn)]);
	if (err != 0) {
...
static int validate_and_get_index(const struct shell *sh, const char *index_arg)
{
	...
	if (index >= CONFIG_BT_CCP_CLIENT_BEARER_COUNT) {
...
static struct bt_ccp_client_bearer *get_bearer_by_index(uint8_t index)
{
	struct bt_ccp_client_bearers bearers;
	...
	err = bt_ccp_client_get_bearers(client, &bearers);
	...
	if (index == 0) {
		...
		return bearers.tbs_bearers[index - 1];
	}
#else
...
static int cmd_ccp_client_read_bearer_name(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (err != 0) {
		shell_error(sh, "Failed to read bearer[%lu] provider name: %d", index, err);
...
SHELL_STATIC_SUBCMD_SET_CREATE(ccp_client_cmds,
